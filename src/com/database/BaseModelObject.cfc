	...
	**/
	...
	public struct function breezeSave( required any entities ){
		...
		//try{
			...
			/* var ret = { "KeyMappings" = keyMappings, "Entities" = arguments.entities, "Errors" = arrayLen( errors ) ? errors : "null" };
			ret = serializeJSON( ret );
			ret = reReplaceNoCase( ret, '"null"', "null", "all" ); */
			var ret = { "KeyMappings" = keyMappings, "Entities" = arguments.entities, "Errors" = errors };
			return ret;
		/*}
		 catch (any e){
			// catastrophic failure. abort. abort.
			throw( e.message );
		}
 */
	}
